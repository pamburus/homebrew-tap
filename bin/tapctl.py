...
def log_command(command, *_args, **kwargs):
    ...
    if context:
        ks = HL.plus(FAINT)
        message += " " + " ".join(
            [ks(f"{k}=") + repr(v) for k, v in context.items()]
        )
...
def log_stdout(
    data: bytes, header: bool = True, body: bool = True, level=logging.DEBUG
):
...
def run(command, *args, **kwargs):
    ...
    log_stdout(
        result.stdout,
        level=logging.INFO if result.returncode else logging.DEBUG,
    )
    log_stderr(
        result.stderr,
        level=logging.INFO if result.returncode else logging.DEBUG,
...
class FormulaAsset:
    ...
    def name(self) -> str:
        ...
        return self.spec["name"].replace("{version}", self.formula.version)
...
class Formula:
    ...
    def sync(self, force=False, write_only=False):
        ...
        params = {
            "VERSION": version,
            **{
                f"ASSET_{x.os.upper()}_{x.arch.upper()}_URL": x.url
                for x in assets
            },
            **{
                f"ASSET_{x.os.upper()}_{x.arch.upper()}_SHA256": x.sha256
                for x in assets
            },
        ...
        if (
            not write_only
            and not self.local_repo.is_clean
            and self.local_repo.is_clean_except(
                self.target_file, self.spec_file
            )
        ):
    ...
    def repo(self):
        ...
        try:
            ...
            raise ValueError(
                f"Invalid repository URL {HL(repr(self.repo_url))} for {self}: {e}"
            ) from e
    ...
    def assets(self, release="latest"):
        ...
        return [
            FormulaAsset(self, spec, release) for spec in self.spec["assets"]
        ]
...
class MultiFormatter(logging.Formatter):
    ...
    def __init__(
        ...
        formatters: typing.Mapping[int, logging.Formatter],
    ):
        super().__init__()
        ...
        formatter = MultiFormatter(
            logging.Formatter("%(message)s"),
            {
                logging.WARNING: logging.Formatter(
                    f"{YELLOW('WARNING:')} %(message)s"
                ),
                logging.ERROR: logging.Formatter(
                    f"{RED('ERROR:')} %(message)s"
                ),
            },
        )
...
def setup_cli_command_sync(group):
    ...
    command = group.add_parser(
        "sync", help="Synchronize the homebrew tap with the source code"
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        error(
            f"Command {HL('`' + shlex.join(err.cmd) + '`')} failed with exit code {RED(err.returncode)}"
        )
